...
abstract contract CircuitBreaker is Ownable {
    ...
    function updateLockdownState(bool state) virtual public{
        inLockdown = state;
    }
}
...
abstract contract ERC20 is ERC20Interface {
  ...
  function allowance(address _owner, address _spender) virtual override view public returns (uint256 remaining) {
...
abstract contract MintableToken is ERC20{
  ...
  function mintToken(address target, uint256 mintedAmount) virtual public returns(bool){
	balances[target] = balances[target].add(mintedAmount);
	totalSupply = totalSupply.add(mintedAmount);
	emit Transfer(address(0), address(this), mintedAmount);
	emit Transfer(address(this), target, mintedAmount);
	emit Minted(target, mintedAmount, block.timestamp);
	return true;
  }
}

// An extension allowing the ERC20 to send any token sent to it to the owner (so they are recoverable)
abstract contract RecoverableToken is ERC20, Ownable {
  
  constructor() {}
   
  event RecoveredTokens(address token, address owner, uint tokens, uint time);
  
  function recoverTokens(ERC20 token) virtual public {
    uint tokens = tokensToBeReturned(token);
    require(token.transfer(getOwner(), tokens) == true);
    emit RecoveredTokens(address(token), getOwner(),  tokens, block.timestamp);
  }
...
abstract contract BurnableToken is ERC20 {
  ...
  function burn(uint256 burnAmount) virtual public {
...
abstract contract WithdrawableToken is ERC20, Ownable {
  using SafeMath for uint256;
  
  event WithdrawLog(uint256 balanceBefore, uint256 amount, uint256 balanceAfter);
  
  function withdraw(uint256 amount) virtual public returns(bool){
...
contract Morality is RecoverableToken, BurnableToken, MintableToken, WithdrawableToken, CircuitBreaker { 
  ...
  constructor(uint256 _totalTokensToMint) payable {
    name = "Morality";
    symbol = "MO";
  ...
  function transfer(address _to, uint256 _value) override public outOfLockdown returns (bool success){
    return super.transfer(_to, _value);
  }
  
  function transferFrom(address _from, address _to, uint256 _value) override public outOfLockdown returns (bool success){
  ...
  function approve(address _spender, uint256 _value) override public outOfLockdown  returns (bool) {
  ...
  function withdraw(uint256 _amount) onlyOwner override  public returns(bool){
	return super.withdraw(_amount);
  }

  function mintToken(address _target, uint256 _mintedAmount) override  onlyOwner public returns (bool){
	return super.mintToken(_target, _mintedAmount);
  }
  
  function burn(uint256 _burnAmount) override onlyOwner public{
    return super.burn(_burnAmount);
  }
  
  function updateLockdownState(bool _state) override onlyOwner public{
    super.updateLockdownState(_state);
  }
  
  function recoverTokens(ERC20 _token) override onlyOwner public{
