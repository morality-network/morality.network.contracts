...
contract StorageToken is Moderatable{
    ...
    uint256[] public uniqueContentIds;
    mapping(uint256 => bool) public contentIdExists;
    mapping(uint256 => uint256[]) public articleIndexes;
    mapping(uint256 => uint256[]) public subArticleIndexes;
    mapping(uint256 => uint256[]) public contentIndexes;
    ...
    function addContent(address contentCreator, uint256 articleId, uint256 subArticleId, uint256 contentId, uint256 timestamp, string memory data) onlyModerators public {
       allContents.push(
           Content(contentCreator, articleId, subArticleId, contentId, timestamp, data)
       );
       uint256 id = allContents.length;
       articleIndexes[articleId].push(id);
       subArticleIndexes[subArticleId].push(id);
       contentIndexes[contentId].push(id);
    ...
    // ------------------------------------------------------------------------
    function getContentsFromIndexes(uint256[] memory contentIndexList) internal view returns(Content[] memory) {
        Content[] memory contents = new Content[](contentIndexList.length);
        for(uint256 i;i<contentIndexList.length;i++){
            uint256 index = contentIndexList[i];
            Content memory contentItem = allContents[index];
            contents[i] = contentItem;
        }
        return contents;
    }
    ...
    // ------------------------------------------------------------------------
    function getContentById(uint256 contentId) public view returns(Content[] memory) {
        uint256[] memory contentIndexList = contentIndexes[contentId];
        return getContentsFromIndexes(contentIndexList);
    }
    ...
    // ------------------------------------------------------------------------
    // Get unique content id count
    // ------------------------------------------------------------------------
    function getUniqueContentIdCount() public view returns(uint256) {
    ...
    // ------------------------------------------------------------------------
    function getAllContentForArticle(uint256 articleId) public view returns(Content[] memory) {
        uint256[] memory contentIndexList = articleIndexes[articleId];
        return getContentsFromIndexes(contentIndexList);
    }
    ...
    function countContentForArticle(uint256 articleId) public view returns(uint256 count) {
        return articleIndexes[articleId].length;
    }
    
    // ------------------------------------------------------------------------
    // Get all content ids for sub-article
    // ------------------------------------------------------------------------
    function getAllContentForSubArticle(uint256 subArticleId) public view returns(Content[] memory) {
        uint256[] memory contentIndexList = subArticleIndexes[subArticleId];
        return getContentsFromIndexes(contentIndexList);
    }
    ...
    function countContentForSubArticle(uint256 subArticleId) public view returns(uint256 count) {
        return subArticleIndexes[subArticleId].length;
    }
